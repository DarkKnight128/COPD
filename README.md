...
2. 系统保存评估结果后，会自动生成一份 `草稿` 报告版本。
3. 医生进入“医生报告复核”页，直接复核和修改报告正文。
4. 保存修改后，系统生成新的报告版本，并将报告状态改为 `待确认`。
5. 医生点击“确认并锁定报告”后，报告状态变为 `已确认`。
6. 已确认报告再次编辑前必须点击“修改报告”，保存后回到 `待确认` 状态，避免直接覆盖已确认版本。
7. 在报告导出页点击“打印 / 导出 PDF”，使用浏览器打印生成基础版慢阻肺智能辅助评估报告。
